              ...
              <section class="vbox">          
                <section class="scrollable padder">
                   <ul class="breadcrumb no-border no-radius b-b b-light pull-in">
                    <li><a href="#"><i class="fa fa-home"></i> Tuyển dụng</a></li>
                </ul>
                <div class="row">
                    <div class="col-xs-12">
                        <div class="panel panel-default">
                            <header class="panel-heading">
                                <a type="btn" class="btn btn-success pull-right btn-sm "  href="{{ url('admin/recruitment/create') }}">Thêm</a>
                                Tuyển dụng
                            </header>
                            ...
                            <tbody>
                                <?php foreach ($recruitment as $key ):?>
                                    <tr>
                                        <th style="width:50px;"> <input type="checkbox" data="$key->id" class="select_id" name="select_id"></th>
                                        <td>{{ $key->name }}</td>

                                        <td>{{ date('d/m/Y', $key->date_deadline) }}</td>
                                        <td><?= ($key->contact);?></td>
                                        <td><?=( $key->state==0) ? 'Hiển thị' :'Ẩn' ?></td>
                                        <td>
                                            <a href="{{ url('admin/recruitment/update') }}<?php echo('/'.$key->id); ?>" title="Edit"> <i class="fa  fa-edit "></i></a>
                                            <a title="close" class="close_id" href="{{ url('admin/recruitment/delete') }}<?php echo('/'.$key['id']); ?>"> <i class="fa  fa-trash-o "></i></a>
                                        </td>
