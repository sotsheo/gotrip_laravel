              ...
              <section class="vbox">          
                <section class="scrollable padder">
                 <ul class="breadcrumb no-border no-radius b-b b-light pull-in">
                    <li><a href="#"><i class="fa fa-home"></i> Banner</a></li>
                </ul>
                <div class="row">
                    <div class="col-xs-12">
                        <div class="panel panel-default">
                            <header class="panel-heading">
                                <a type="btn" class="btn btn-success pull-right btn-sm "  href="{{ url('admin/banner/create') }}">Thêm</a>
                               Banner
                            </header>
                                ...
                                <tbody>
                                    <?php foreach ($banner as $key ):?>
                                        <tr>
                                            ...
                                            <td>{{ $key->name }}</td>
                                            <td><img src="<?= url($key->img_root)?>" style="max-height: 50px;"/></td>  
                                            <td>
                                            ...
                                            <td><?=( $key->status!=0) ? 'Hiển thị' :'Ẩn' ?></td>     
                                            <td>{{ date('d/m/Y', $key->created_at) }}</td>
                                            <td><a href="{{ url('admin/banner/update') }}<?php echo('/'.$key->id); ?>" title="Edit"> <i class="fa  fa-edit "></i></a> <a title="close" class="close_id" href="{{ url('admin/banner/delete') }}<?php echo('/'.$key['id']); ?>"> <i class="fa  fa-trash-o "></i></a></td>
