              ...
              <section class="vbox">          
                <section class="scrollable padder">
                 <ul class="breadcrumb no-border no-radius b-b b-light pull-in">
                    <li><a href="#"><i class="fa fa-home"></i>  Danh mục banner</a></li>
                </ul>
                <div class="row">
                    <div class="col-xs-12">
                        <div class="panel panel-default">
                            <header class="panel-heading">
                                <a type="btn" class="btn btn-success pull-right btn-sm "  href="{{ url('admin/category_banner/create') }}">Thêm</a>
                                Danh mục banner
                            </header>
                                ...
                                <tbody>
                                    <?php foreach ($category_banner as $key ):?>
                                        <tr>
                                            ...
                                            <td>{{ $key->name }}</td>
                                            <td><?=( $key->state!=1) ? 'Hiển thị' :'Ẩn' ?></td>     
                                            <td>{{ $key->short_description }}</td>
                                            <td><?=  date('d/m/Y', $key->created_at ); ?></td>
                                            <td><a href="{{ url('admin/category_banner/update') }}<?php echo('/'.$key->id); ?>" title="Edit"> <i class="fa  fa-edit "></i></a> <a title="close" class="close_id" href="{{ url('admin/category_banner/delete') }}<?php echo('/'.$key['id']); ?>"> <i class="fa  fa-trash-o "></i></a></td>
