    ...
    <div class="form-group">
        ...
        <div class="col-sm-10 control-label">
            <input type="file"  name="attachment[]"  multiple  id="uploadfile">
            <div id="sortable">
                <?php
                if(isset($images) && $images){
                    foreach($images as $item){
                        ...
                        <div style="float: left;margin-right: 10px;margin-top: 10px;" class="img_list">
                            <div class="contents">
                                <img src="<?= $item['img_path'].'/200x200/'.$item['img_name']?>"  class="img-thumbnail" style='height: 250px;width: 250px;'>
...
<script>
    $(document).ready(function(){
        ...
        function upload(img) {
            ...
            form_data.append('id', $('#id_product').val());
            form_data.append('file', img.files[0]);
            form_data.append('count', img.files.length);
            form_data.append('_token', '{{csrf_token()}}');
            $('#loading').css('display', 'block');
            $.ajax({
                url: "{{route('uploadFileAlbum')}}",
                data: form_data,
                type: 'POST',
                contentType: false,
                processData: false,
