                    ...
                    <div class="col-xs-12">
                        <div class="panel panel-default">
                            <header class="panel-heading">
                                <a type="btn" class="btn btn-success pull-right btn-sm "  href="{{ url('admin/combo/category/create') }}">Thêm</a>
                                Combo
                            </header>
                                        ...
                                        <tbody>
                                            <?php foreach ($category as $key=>$val ):?>
                                                <tr>
	                                                ...
	                                                <?php if($val->img_root){
	                                                    ?>
	                                                    <img src="<?= url($val->img_root)?>" style="max-width: 50px;"/>
	                                                <?php }?>
	                                                </td>  
                                                    <td><?=( $val->status) ? 'Hiển thị' :'Ẩn' ?></td>
                                                    <td>
                                                       
                                                        <a href="{{ url('admin/combo/category/update') }}<?php echo('/'.$val->id); ?>" title="Edit"> <i class="fa  fa-edit "></i></a> 

                                                        <a title="close" class="close_id" href="{{ url('admin/combo/category/delete') }}<?php echo('/'.$val->id); ?>"> <i class="fa fa-trash-o"></i></a>
                                                    </td>
